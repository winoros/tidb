...
| Claiming task completion / PR readiness | MUST use the `Ready` verification profile from `.agents/skills/tidb-verify-profile`; if there are code changes, `Ready` includes `make lint`. Before finishing, `Ready` also includes a self-review of diff quality following the `tidb-code-style` skill's comment review guidance (`code-comment-review-guide.md`). `Ready` is mandatory before making final-status claims such as "fixed", "done", "all tests pass", "ready for review", or "ready for PR". |
...
1. Files changed.
2. Whether comment quality meets the `tidb-code-style` skill's `code-comment-style-guide.md`, and whether the diff was self-reviewed against `code-comment-review-guide.md`.
