...
func (p *PhysicalTableScan) GetPlanCostVer2(taskType property.TaskType, option *optimizetrace.PlanCostOption) (costusage.CostVer2, error) {
	...
	if p.AnnIndexExtra != nil {
		if p.AnnIndexExtra.TopK == math.MaxUint32 {
			...
			p.PlanCostVer2 = costusage.SumCostVer2(p.PlanCostVer2, costusage.NewCostVer2(option, defaultVer2Factors.ANNIndexScanRow, float64(p.AnnIndexExtra.TopK)*defaultVer2Factors.ANNIndexScanRow.Value, func() string {
				return fmt.Sprintf("ann-index-topk(%v*%v)", p.AnnIndexExtra.TopK, defaultVer2Factors.ANNIndexScanRow)
			}))
		}
	}
