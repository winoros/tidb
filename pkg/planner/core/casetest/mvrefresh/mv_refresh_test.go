...
func TestExplainRefreshMVFastPlanTreeMinMax(t *testing.T) {
	...
	require.Equal(t, [][]string{
		...
		{"│ ├─HashAgg(Build)", "6400.00", "root", "", "group by:Column#48, funcs:firstrow(Column#39)->Column#18, funcs:firstrow(Column#40)->Column#19, funcs:sum_int(Column#41)->Column#20, funcs:firstrow(Column#42)->Column#21, funcs:sum_int(Column#43)->Column#22, funcs:firstrow(Column#44)->Column#23, funcs:sum_int(Column#45)->Column#24, funcs:firstrow(Column#46)->Column#25, funcs:sum_int(Column#47)->Column#26, funcs:firstrow(Column#48)->test.$mlog$t.a"},
		{"│ │ └─Projection", "8000.00", "root", "", "Column#7->Column#39, Column#8->Column#40, if(and(eq(test.$mlog$t._mlog$_old_new, 1), eq(test.$mlog$t.b, Column#8)), 1, 0)->Column#41, Column#9->Column#42, if(and(eq(test.$mlog$t._mlog$_old_new, -1), eq(test.$mlog$t.b, Column#9)), 1, 0)->Column#43, Column#10->Column#44, if(and(eq(test.$mlog$t._mlog$_old_new, 1), eq(test.$mlog$t.b, Column#10)), 1, 0)->Column#45, Column#11->Column#46, if(and(eq(test.$mlog$t._mlog$_old_new, -1), eq(test.$mlog$t.b, Column#11)), 1, 0)->Column#47, test.$mlog$t.a->Column#48"},
		{"│ │   └─HashJoin", "8000.00", "root", "", "left outer join, equal:[nulleq(test.$mlog$t.a, test.$mlog$t.a)]"},
		{"│ │     ├─HashAgg(Build)", "6400.00", "root", "", "group by:test.$mlog$t.a, funcs:sum_int(Column#33)->Column#7, funcs:max(Column#34)->Column#8, funcs:max(Column#35)->Column#9, funcs:min(Column#36)->Column#10, funcs:min(Column#37)->Column#11, funcs:firstrow(test.$mlog$t.a)->test.$mlog$t.a"},
		{"│ │     │ └─TableReader", "6400.00", "root", "", "data:HashAgg"},
		{"│ │     │   └─HashAgg", "6400.00", "cop[tikv]", "", "group by:test.$mlog$t.a, funcs:sum_int(test.$mlog$t._mlog$_old_new)->Column#33, funcs:max(if(eq(test.$mlog$t._mlog$_old_new, 1), test.$mlog$t.b, NULL))->Column#34, funcs:max(if(eq(test.$mlog$t._mlog$_old_new, -1), test.$mlog$t.b, NULL))->Column#35, funcs:min(if(eq(test.$mlog$t._mlog$_old_new, 1), test.$mlog$t.b, NULL))->Column#36, funcs:min(if(eq(test.$mlog$t._mlog$_old_new, -1), test.$mlog$t.b, NULL))->Column#37"},
		{"│ │     │     └─Selection", "8000.00", "cop[tikv]", "", "gt(test.$mlog$t._tidb_commit_ts, 0), le(test.$mlog$t._tidb_commit_ts, 1)"},
		{"│ │     │       └─TableFullScan", "10000.00", "cop[tikv]", "table:$mlog$t", "keep order:false, stats:pseudo"},
